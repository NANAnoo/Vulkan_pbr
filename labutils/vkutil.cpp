...
namespace labutils
{
	ShaderModule load_shader_module( VulkanContext const& aContext, char const* aSpirvPath )
	{
		...
		if (std::FILE* fin = std::fopen(aSpirvPath, "rb")) {
			...
			// SPIR-V consists of a number of 32-bit = 4 byte words 9
			assert(0 == bytes % 4);
	...
	ImageView create_image_view_texture2d( VulkanContext const& aContext, VkImage aImage, VkFormat aFormat) {
		...
		if ( auto const res = vkCreateImageView( 
				aContext.device,
				&viewInfo,
				nullptr,
				&view); 
	...
	Sampler create_anisotropic_sampler(VulkanContext const& aContext, float max)
	{
		VkSamplerCreateInfo samplerInfo{};
		samplerInfo.sType = VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO;
		samplerInfo.magFilter = VK_FILTER_LINEAR;
	...
	void buffer_barrier(VkCommandBuffer aCmdBuff, VkBuffer aBuffer, 
